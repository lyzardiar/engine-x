 ...
 * https://cloud.tencent.com/developer/article/1005839
 * 
寻找(Seek)
...
抵达(Arriving)
	抵达目的是进入目标的一个范围之内，在范围之外和寻找一样朝目标移动，
	进入一定距离后逐渐减速（由timeToTarget和maxSpeed决定何时减速），进入目标的半径范围内静止。
...
追逐(Pursuit) 躲避(Evade) 
	如果我们在追逐一个移动的目标，那么只是朝向他当前位置将不再有效。在角色抵达目标当前位置时，
	目标已经离开了，如果他们距离足够近还好，如果它们之间距离足够远，我们就需要预测目标的未来位置，
	这就是追组行为。他使用探寻(Seek)这个基础行为，假设目标将以当前速度保持移动进行预测将来位置
...
路径跟随(Path Following)
	路径跟随是一个以一整条路径作为目标的转向行为。一个拥有路径跟随行为的角色应该沿着路径的一个方向移动。
	他也是一个代理行为，根据当前角色的位置和路径来计算出目标的位置，然后使用探索(Seek)行为去移动。
	目标位置的计算有两个阶段。第一，将角色位置转换到路径上的一个最近点；
	第二，在路径上选中一个目标而不是路径上的一个固定距离的映射点
	（a target is selected which is further along the path than the mapped point by a fixed distance)。

分离(Separation)
	分离行为针对于拥有大致相同方向的一堆目标，使他们保持距离。但是它在目标相互移动穿插的情况无法作用，
	这时候需要之后介绍的碰撞避免行为。
...
障碍和墙躲避(Obstacle And Wall Avoidance)
	碰撞躲避行为假设目标是球形的，它关注于躲避目标的中心位置。而障碍和墙躲避行为的目标形状更复杂，
	所以实现方式也不一样。通过从移动的角色前方发射一条或者多条有限长度的射线，如果碰撞到障碍，
	角色就开始进行躲避，根据碰撞信息获得一个移动的目标，进行探寻行为。

拐角困境(The Corner Trap)
	多条射线躲避墙壁算法会遭遇到尖锐的夹角障碍的问题，导致朝任意两边移动另一条射线都会碰撞到墙面，最终仍然撞向障碍物
...
public class AutoPilot implements Cloneable, SBF{
	 ...
	 * @param vehicle
	 */
	public AutoPilot setOwner(const Vehicle vehicle){
		this.owner = vehicle;
		return this;
	}
	 ...
	 * @return array of whisker vectors
	 */
	public Vector2D[] getFeelers(const Vector2D  facing, const Vector2D  origin ){
		return createWhiskers( nbrWhiskers, // Number of Whiskers
				whiskerLength, // Whisker length
				whiskerFOV, // fov
				facing, // facing
	...
	private Vector2D[] createWhiskers(
			const int  		nbrWhiskers,
			const double    whiskerLength,
			const double    fov,
			const Vector2D  facing,
