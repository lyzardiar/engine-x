...
class Vector2D : public Serializable {
	private: static const long serialVersionUID = -7691691832344874590L;

	/** Null vector (coordinates: 0, 0). */
	public: static const Vector2D ZERO = Vector2D(0, 0);

	/** Null vector (coordinates: 1, 1). */
	public: static const Vector2D ONE = Vector2D(1, 1);

	/** First canonical vector (coordinates: 1, 0). */
	public: static const Vector2D PLUS_I = Vector2D(1, 0);

	/** Opposite of the first canonical vector (coordinates: -1, 0). */
	public: static const Vector2D MINUS_I = Vector2D(-1, 0);

	/** Second canonical vector (coordinates: 0, 1). */
	public: static const Vector2D PLUS_J = Vector2D(0, 1);

	/** Opposite of the second canonical vector (coordinates: 0, -1). */
	public: static const Vector2D MINUS_J = Vector2D(0, -1);

	/** A vector with all coordinates set to NaN. */
	public: static const Vector2D NaN = Vector2D(Double::NaN, Double::NaN);

	/** A vector with all coordinates set to positive infinity. */
	public: static const Vector2D POSITIVE_INFINITY =
		Vector2D(Double::POSITIVE_INFINITY, Double::POSITIVE_INFINITY);

	/** A vector with all coordinates set to negative infinity. */
	public: static const Vector2D NEGATIVE_INFINITY =
		Vector2D(Double::NEGATIVE_INFINITY, Double::NEGATIVE_INFINITY);

	public: static const double EPSILON = 1e-10;
	
	public: static const int CLOCKWISE = 1;
	public: static const int ANTI_CLOCKWISE = -1;

	public: double x;
	public: double y;


	public: static bool areEqual(Vector2D v0, Vector2D v1){
	...
	public: void set(const Vector2D v){
		this.x = v.x;
		this.y = v.y;		
	}
	 ...
	 */
	public: double dot(const Vector2D v){
		return (x*v.x + y*v.y);
	}
	 ...
	 */
	public: double dotNorm(const Vector2D v){
	 ...
	 */
	public: double angleBetween(const Vector2D v){
		double denom = FastMath.sqrt(x * x + y * y) * FastMath.sqrt(v.x * v.x + v.y * v.y);
	 ...
	 */
	public: int sign(const Vector2D v){
		if(y*v.x > x*v.y)
			return CLOCKWISE;
		else
	 ...
	 */
	public: double distanceSq(const Vector2D v){
		double dx = v.x - x;
		double dy = v.y - y;
		return dx*dx + dy*dy;
	}
	 ...
	 */
	public: double distance(const Vector2D v){
		double dx = v.x - x;
		double dy = v.y - y;
	 ...
	 */
	public: Vector2D getReflect(const Vector2D norm){
	 ...
	 */
	public: Vector2D add(const Vector2D v){
		x += v.x;
		y += v.y;
		return this;
	}
	...
	public: Vector2D sub(const Vector2D v){
		x -= v.x;
		y -= v.y;
		return this;
	}
	 ...
	 */
	public: static Vector2D add(const Vector2D v0, const Vector2D v1){
	 ...
	 */
	public: static Vector2D sub(const Vector2D v0, const Vector2D v1){
		return new Vector2D(v0.x - v1.x, v0.y - v1.y);
	}
	 ...
	 */
	public: static Vector2D mult(const Vector2D v, double d){
	 ...
	 * @return the calculated vector
	 */
	public: static Vector2D div(const Vector2D v, double d){
		return new Vector2D(v.x / d, v.y / d);
	}
	 ...
	 */
	public: static double distSq(const Vector2D v0, const Vector2D v1){
		double dx = v1.x - v0.x;
		double dy = v1.y - v0.y;
		return dx*dx + dy*dy;
	}
	 ...
	 */
	public: static double dist(const Vector2D v0, const Vector2D v1){
		double dx = v1.x - v0.x;
		double dy = v1.y - v0.y;
		return FastMath.sqrt(dx*dx + dy*dy);
	}
	 ...
	 */
	public: static Vector2D normalize(const Vector2D v){
		Vector2D n;
		double mag = v.length();
		if(mag < Double.MIN_VALUE)
			n = new Vector2D();
	 ...
	 */
	public: static bool isSecondInFOVofFirstXXX(const Vector2D posFirst, const Vector2D facingFirst, const double fovFirst, const Vector2D posSecond){
